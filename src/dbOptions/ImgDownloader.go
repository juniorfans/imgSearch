...
func singleSaveImages(img_url string, goId int, maxImgId int, imgId int) int {
    ...
    if err != nil || nil == response || 200 != response.StatusCode {
...
func save(goId int, base int, times int)  {
    ...
    for i := 0; i != times; i++{
        ...
        startT := time.Now().Unix()
        nextImgId = saveImages("https://kyfw.12306.cn/passport/captcha/captcha-image?login_site=E&module=login", goId, maxImageId, nextImgId)
