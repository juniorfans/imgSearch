...
/**
	从 db 库(clip reverse index)中获得上一次各线程最后处理的图片的 id 和各线程处理的总图片数目 count
	id 在 db 中的键名：ZLAST_i_tid， i 是图片库 id， tid 为线程 id
	count 在 db 中的键名：ZLAST_C_i_tid， i 是图片库 id， tid 为线程 id

 */
func GetThreadLastDealedKey(db *DBConfig, dbIndex uint8, threadId int) (lastDealedKey []byte , count int){
	...
	countStr, err := db.DBPtr.Get([]byte(key), &db.ReadOptions)
	if err == leveldb.ErrNotFound{
		count = 0
		return
	}
